  ...
  auth: router({
    ...
    resetPassword: publicProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (tokenRecord.used) {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Token já utilizado' });
        }
    ...
    changePassword: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        const { comparePassword, hashPassword } = await import('./auth-helpers');

        const user = await db.getUserById(ctx.user.id);
        if (!user || !user.password) {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Usuário não encontrado ou sem senha cadastrada' });
        }
  ...
  appointments: router({
    ...
    cancel: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (appointment.status === 'cancelled') {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Agendamento já cancelado' });
        }

        if (appointment.status === 'completed') {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Não é possível cancelar aula já realizada' });
        }
    ...
    getAvailableSlots: publicProcedure
      ...
      .query(async ({ input }) => {
        ...
        // Define available hours (e.g., 6:00 to 22:00)
        ...
        for (let hour = 6; hour <= 22; hour++) {
          ...
          const appointmentsAtThisTime = existingAppointments.filter(apt =>
      ...
      .query(async ({ input }) => {
        ...
        if (input.startDate) {
          appointments = appointments.filter(apt =>
            new Date(apt.appointmentDate) >= input.startDate!
          );
        }
        if (input.endDate) {
          appointments = appointments.filter(apt =>
            new Date(apt.appointmentDate) <= input.endDate!
          );
        }
    ...
    markCompleted: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (appointment.status !== 'scheduled') {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Apenas agendamentos com status "scheduled" podem ser marcados como completados' });
        }
    ...
    markNoShow: adminProcedure
      ...
      .mutation(async ({ input }) => {
        ...
        if (appointment.status !== 'scheduled') {
          throw new TRPCError({ code: 'BAD_REQUEST', message: 'Apenas agendamentos com status "scheduled" podem ser marcados como no-show' });
        }
